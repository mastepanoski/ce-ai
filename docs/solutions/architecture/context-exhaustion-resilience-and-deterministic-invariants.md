...
- **Git Hooks Probe**: Checks `git config --get core.hooksPath`. Normalizes Windows/Unix path separators (stripping trailing `/` and `\`) to ensure hooks route to `.githooks`. Verifies that `.githooks/pre-commit` exists.
  - ⚠️ **Worktree-contention caveat (2026-08-22)**: `.git/config` is shared by all worktrees of a repo. During parallel multi-agent sessions, concurrent agents mutating `core.hooksPath`, `core.bare`, or `extensions.worktreeConfig` make this probe (and worktree detection itself) non-deterministic. When doctor findings appear/disappear between runs without code changes, diff `git config -l --local` across contexts before debugging code — see the multi-agent guidance in `docs/solutions/workflow-issues/`.
