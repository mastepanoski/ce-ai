...
### Added
- **Self-Explaining PR Directives & Upfront Review Readiness in AGENTS.md Managed Blocks (#423)**:
  - **"A Change That Can Explain Itself" Directives**: Enhanced `render_block_content` in `src/commands/init_prj.rs` across `Full`, `Minimal`, and `Orchestrator` adoption tiers, mandating that PRs generated by AI agents explicitly explain *what alternatives were explored and ruled out* (from OpenSpec `exploration.md`) and *which specific rule, invariant, or architectural constraint decided the approach* (from `AGENTS.md` / `design.md`).
  - **Upfront Verification Evidence**: Mandated attaching empirical validation proof (test run summaries, CLI logs, reproduction traces, or browser check results) directly to the PR body using collapsible `<details><summary>` blocks (`not asked for later`), preserving the ~100–150 line description cognitive budget without bloating code diff line counts.
  - **Pre-Review Automated Checks Gate**: Mandated that all local linters, unit tests, and browser/E2E checks must run to 100% green before requesting human review or marking the PR ready.
  - **Reviewer Routing**: Required identifying subsystem owners or `CODEOWNERS` to assign appropriate reviewers based on domain boundaries.
  - **Managed Block Version Bump (`BLOCK_VERSION: 6`)**: Bumped adoption block schema version from 5 to 6 in `src/commands/init_prj.rs`, enabling `ce-ai doctor` and `ce-ai status` to classify v5 blocks as `StaleVersion` with actionable upgrade guidance (`re-run ce-ai init-prj --tier <tier> to upgrade`).
  - **Test Fixture Coordination**: Synchronized `CUR_BLOCK_VERSION = 6` in `tests/cli.rs` and added regression tests for upgrading stale v5 blocks to v6 with self-explaining directives.
  - **Governance & Policy Documentation**:
    - Updated root `AGENTS.md` Invariant #7 and Definition of Done (DoD) to enforce self-explaining PRs and upfront evidence.
    - Updated `CONTRIBUTING.md` § "PR Size Boundaries & Bounded Corrections" to clarify that collapsible evidence in PR descriptions does not count against code LOC limits, and affirmed that the 400 LOC review boundary is strictly preserved.
...
- **Documentation Technical Debt Diagnostic Engine & Probes:**
  ...
  - **Substrate Tri-State Reporting**: Implemented `ProbeStatus::Clean`, `ProbeStatus::Debt(findings)`, and `ProbeStatus::Unknown` (`[git: n/a]`), preventing silent false negatives when Git is unavailable.
  ...
  - **Workspace Configuration Overrides**: Support for `doc_hygiene` in `.ce-ai.json` allowing per-repo configuration of `stale_spec_days` (default 21), `check_solution_paths` (default true), and `require_solution_frontmatter` (default true).
...
- **`init-prj --dry-run` Reported Fake Adoption and Hid Hook Actions (#351):**
  ...
  - Detection parity: dry-run previews the Claude RTK hook on a fresh project (a real run always creates the `CLAUDE.md` stub), even when `.claude` does not yet exist.
  - Dry-run remains fully non-destructive: zero writes to the target repository, harness config, or `state.json`, and a corrupt global `state.json` no longer aborts a write-free preview.
  - Added CLI regression tests locking in the preview wording, zero-write invariant, fresh-project parity, quiet mode, up-to-date reporting, and opt-out behavior.
...
### Added
- **Native CodeGraph Index Initialization & Project Adoption Support (`ce-ai tools init`, `ce-ai init-prj`):**
  - Added `ce-ai tools init <tool> [path]` subcommand supporting native CodeGraph index creation (`ce-ai tools init codegraph`), validating binary availability, handling dry-run execution, and skipping gracefully when `.codegraph/` already exists.
  - Added automatic CodeGraph index initialization during `ce-ai init-prj` when `codegraph` is present on `PATH` and `.codegraph/` is missing in the adopted workspace.
  - Updated `ce-ai doctor` to suggest `'ce-ai tools init codegraph'` when `.codegraph/` index is uninitialized.

### Fixed
- **Cleaned Up Legacy `gentle-ai` Residuals in Guidance and Documentation:**
  - Updated `ce-ai audit` `CodeIntelDetector` detail message to suggest `codegraph init` or `ce-ai tools init codegraph` instead of external `gentle-ai codegraph init` (`src/commands/audit.rs`).
  - Replaced erroneous `gentle-ai:ce-ai-ignore` sentinel comments with canonical `# BEGIN CE-AI MANAGED BLOCK` in OpenSpec exploration docs (`openspec/changes/workspace-scoped-workflow-and-gitignore/exploration.md`).
  - Updated `docs/user-guide/quick-start-workflow-guide.md` to reference `codegraph init` and `ce-ai tools init codegraph`.
