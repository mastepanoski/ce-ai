...
- **OS Keyring Integration & Secure Stdin Input for Decisions Auth**:
  ...
  - **Interactive Masked Terminal Ingestion (`rpassword`)**: Running `ce-ai decisions auth --key` (without a value) securely prompts for the API key in interactive terminals with characters hidden, printing a masked confirmation without leaving secrets in the terminal buffer.
