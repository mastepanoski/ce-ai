...
- [x] 1. Update `RegistrationSpec` struct and implement directory guard in `src/harness/registration.rs` (~35 LOC)
...
- [x] 2. Unit tests in `src/harness/tests/registration.rs` (~45 LOC)
...
- [x] 3. CLI integration test in `tests/cli.rs` (~65 LOC)
...
- [x] 4. Documentation and Solution architecture capture (~60 LOC)
  - Create solution architecture doc `docs/solutions/architecture/install-directory-config-mcp-guard.md`.
- [x] 5. SemVer bump and CHANGELOG update (~15 LOC)
