...
- [x] 1. Implement reconciliation data structures and extraction logic in `src/commands/workflow.rs` (~120 LOC)
...
- [x] 2. Integrate reconciliation into `RepoState`, `resume_lines`, `status_lines`, and `checkpoint_lines` (~50 LOC)
...
- [x] 3. Add diagnostic probe in `src/commands/doctor.rs` (~30 LOC)
...
- [x] 4. Unit tests in `src/commands/tests/workflow.rs` (~110 LOC)
  ...
  - Test graceful degradation returning `None` without panic/failure when git is unavailable or non-git workspace (R7).
  - Test `warning_line` formatting.
- [x] 5. CLI integration tests in `tests/cli.rs` (~80 LOC)
...
- [x] 6. Documentation and Solution Architecture capture (~40 LOC)
...
- [x] 7. SemVer version bump and CHANGELOG update (~15 LOC)
