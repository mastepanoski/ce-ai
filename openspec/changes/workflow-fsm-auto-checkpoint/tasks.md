...
- [x] Add `WorkflowSource` enum (`Manual`, `Inferred`) with `Default` returning `Manual`.
- [x] Extend `WorkflowState` struct with `pub source: WorkflowSource` (`#[serde(default)]` for backward compatibility).
- [x] Implement `State::workspace_branch_key(root: &Path, branch: Option<&str>) -> String`.
- [x] Implement `State::current_workflow_for_branch(&self, root: &Path, branch: Option<&str>) -> Option<WorkflowState>` with hierarchical fallback (`<canonical_root>::<branch>` -> `<canonical_root>` -> `self.workflow`).
- [x] Implement `State::atomic_update_workflow` with reload-before-save check to eliminate multi-turn read-modify-write races on `state.json`.
- [x] TDD Unit Tests:
  - Verify branch composite key creation and fallback.
  - Verify reload-before-save check prevents clobbering when concurrent updates occur.
...
- [x] Implement `is_transitory_git_state(repo_root: &Path) -> bool` checking for `.git/rebase-merge`, `.git/rebase-apply`, `.git/CHERRY_PICK_HEAD`, and `.git/MERGE_HEAD`.
- [x] Implement `sanitize_feature_name(branch: &str) -> String` to prevent path traversal outside `openspec/changes/`.
- [x] Implement `infer_stage_from_repo(repo_root: &Path, branch: Option<&str>) -> Option<(WorkflowStage, String, Option<String>)>` mapping observable repository state to Stages 1 through 7.
- [x] Enforce monotonic progression: inferred stages advance state only when `can_transition_to` is valid and never regress a `WorkflowSource::Manual` checkpoint.
- [x] Wire inference into `ce-ai workflow resume` and `ce-ai workflow status`.
- [x] Support `auto_checkpoint: bool` configuration toggle (skip persistence when disabled).
- [x] TDD Unit Tests:
...
- [x] Update `src/harness/claude.rs`:
...
- [x] Update `src/opencode/plugins.rs` and `.opencode/plugins/compound-engineering.js`:
...
- [x] Update `src/commands/init_prj.rs`:
  - Support updating hooks across all harnesses when `force == true` or when existing hook has stale version / missing keys.
- [x] Wire hook verification and refresh into `src/commands/sync.rs` and `src/commands/upgrade.rs`.
- [x] Update `src/commands/deinit_prj.rs`:
...
- [x] Add CLI integration test: `workflow_auto_checkpoint_infers_stage_on_resume_and_status`.
- [x] Add CLI integration test: `workflow_branch_scoping_prevents_cross_branch_clobbering`.
- [x] Add CLI integration test: `workflow_transitory_git_state_suppresses_auto_checkpoint`.
- [x] Add CLI integration test: `workflow_inferred_stage_never_regresses_manual_checkpoint`.
- [x] Add CLI integration tests: roundtrip `init-prj` -> `deinit-prj` verifying 100% clean removal without orphans across:
...
- [x] Update `docs/user-guide/fsm-and-checkpoints-explained.md`:
...
- [x] Verify `README.md` length constraint (<= 100 lines) and clarify wording regarding adoption-level opt-in.
