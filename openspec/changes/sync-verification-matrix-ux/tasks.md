...
- [ ] **T1.1 (RED)** Add unit tests to `src/commands/sync.rs` `mod tests`
...
- [ ] **T1.2 (GREEN)** Implement the four pure functions per design.md; add
      ...
      when `unverified > 0`.
- [ ] **T1.3** Verify: `cargo test` green, `cargo fmt --check` clean,
      `cargo clippy --all-targets --all-features -- -D warnings` clean.

## Work Unit 2 — Docs + version + changelog (~80 LOC)

- [ ] **T2.1** Update `docs/user-guide/sync-and-upgrade-mechanisms.md` Step 6:
...
- [ ] **T2.2** Bump `Cargo.toml` to `1.22.4`; add `CHANGELOG.md` `[1.22.4]`
      `### Changed` entry describing the matrix wording and guidance note.
- [ ] **T2.3** Final gate before PR: full `cargo test`, fmt, clippy; run
