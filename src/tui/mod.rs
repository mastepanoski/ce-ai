                ...
                event::read().map_err(|e| CeError::Runtime(format!("read error: {e}")))?
            {
                ...
                match key.code {
                    ...
                    KeyCode::Char('i') | KeyCode::Char('I') => {
                        let lines = handlers::run_init_prj_cmd(ctx);
                        ...
                        match app.current_tab() {
                            MenuTab::Status => {
                                execute_action(app, "Harness Status", || {
                                    handlers::run_status_cmd(ctx)
                                });
                            }
                            MenuTab::Workflow => {
                                execute_action(app, "Workflow FSM Status", || {
                                    handlers::run_workflow_cmd(ctx)
                                });
                            }
                            MenuTab::Install => {
                                let lines = handlers::run_install_cmd(ctx, app, dry_run);
                                execute_action(app, "Install Plugin", move || lines);
                            }
                            MenuTab::Models => {
                                execute_action(app, "Model Assignments", || {
                                    handlers::run_models_cmd(ctx)
                                });
                            }
                            MenuTab::Skills => {
                                execute_action(app, "Skills Registry", || {
                                    handlers::run_skills_cmd(ctx)
                                });
                            }
                            MenuTab::Sync => {
                                execute_action(app, "Sync Drift", move || {
                                    handlers::run_sync_cmd(ctx, dry_run)
                                });
                            }
                            MenuTab::Upgrade => {
                                let lines = handlers::run_upgrade_cmd(ctx, app);
                                execute_action(app, "Upgrade Release", move || lines);
                            }
                            MenuTab::Doctor => {
                                execute_action(app, "Doctor Diagnostics", || {
                                    handlers::run_doctor_cmd(ctx)
                                });
                            }
                            MenuTab::Backups => {
                                let lines = handlers::run_restore_backup_cmd(ctx, app);
                                execute_action(app, "Restore Backup", move || lines);
                            }
                            MenuTab::Tools => {
                                execute_action(app, "Tools Status", || {
                                    handlers::run_tools_cmd(ctx)
                                });
                            }
                            MenuTab::Usage => {
                                execute_action(app, "Usage Report", || {
                                    handlers::run_usage_cmd(ctx)
                                });
                            }
                            MenuTab::Audit => {
                                execute_action(app, "Audit", || handlers::run_audit_cmd(ctx));
                            }
                            MenuTab::InitPrj => {
                                let lines = handlers::run_init_prj_cmd(ctx);
                            ...
                            MenuTab::Uninstall => {
                                let lines = handlers::run_uninstall_cmd(ctx, app);
                    ...
                    KeyCode::Char(c @ '1'..='7') if app.current_tab() == MenuTab::Workflow => {
                        ...
                        let lines = handlers::workflow_stage_transition_lines(ctx, stage_num);
...
mod tests {
    ...
    fn failure_lines_carry_marker_and_remedy() {
        let lines =
            handlers::workflow_failure_lines(&CeError::Runtime("state.json corrupt".into()));
